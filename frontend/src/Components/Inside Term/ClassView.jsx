...
export default function ClassView(url) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`https://gsbgs-backend.vercel.app/api/classes/${url}`);
    ...
    >
      ...
      sx={{
        ...
        <Box sx={{ display: "flex", flexDirection:'column', justifyContent: 'center', alignItems: 'center'}}>
          <Typography fontWeight={"bold"} sx={{fontSize: {xs: '1.5rem', lg: '2rem'}}}>
            Teacher
          </Typography>
        ...
        <Box sx={{ display: "flex", marginTop: "15px", justifyContent: 'center', alignContent: 'center' }}>
