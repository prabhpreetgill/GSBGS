...
export default function UnPaidTable({ month, term }) {
  ...
  const unpaidStudents = data?.filter((student) => {
    if (student?.paymentPlan !== "Monthly") return false;

    // Safely check if payment for the term and month exists
    const termPayments = student._payments?.[term] || {};
    return !termPayments[month];
  });

  const filteredData = unpaidStudents?.filter((row) => {
